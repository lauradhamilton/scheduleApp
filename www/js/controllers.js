...
.controller('SplashCtrl', function($scope, $state, $stateParams, $filter, $http) {
  ...
  $scope.goToToday = function() {
    $http.post("http://www.additiveanalytics.com/api_sessions", {"email": $scope.user.email,"password": $scope.user.password})
...
.controller('ScheduleCtrl', function($scope, $http, $state, $stateParams, $filter, $timeout) {
  ...
  $http.get('http://www.additiveanalytics.com/api/schedule_app_api?appointment_date=' + $stateParams.date + "&attending_abbreviation=" + localStorage.getItem("attending_abbreviation") + "&user_email=" + localStorage.getItem("user_email") + "&user_token=" + localStorage.getItem("user_token"))
    .success(function(data){
